app.config(['$routeProvider', function ($routeProvider) {
    $routeProvider
        ...
        .when('/shop', {
            templateUrl: "keeper/Shop/index.html",
            controller: "ShopCtr"
        }) //门店员工展示
        .when('/shop/add', {
            templateUrl: "keeper/Shop/add.html",
            controller: "ShopAddCtr"
        }) //门店添加第一步
        .when('/shop/add2', {
            templateUrl: "keeper/Shop/add2.html",
            controller: "ShopAdd2Ctr"
        }) //门店添加第二步
        .when('/shop/add3', {
            templateUrl: "keeper/Shop/add3.html",
            controller: "ShopAdd3Ctr"
        }) //门店添加第三步
        .when('/shop/add4', {
            templateUrl: "keeper/Shop/add4.html",
            controller: "ShopAdd4Ctr"
        }) //门店添加第四步
        .when('/shop/add5', {
            templateUrl: "keeper/Shop/add5.html",
            controller: "ShopAdd5Ctr"
        }) //门店添加第四步
        .when('/shop/edit', {
            templateUrl: "keeper/Shop/edit.html",
            controller: "ShopAddCtr"
        }) //门店修改
        .when('/shop/businesstime', {
            templateUrl: "keeper/Shop/businesstime.html",
            controller: "ShopBusinesstimeCtr"
        }) //门店营业时间修改
        .when('/shop/add2re', {
            templateUrl: "keeper/Shop/add2re.html",
            controller: "TablesCtr"
        }) //添加桌台返回

        .when('/tables', {
            templateUrl: "keeper/Shop/tables.html",
            controller: "TablesCtr"
        }) //桌台管理
        .when('/table/tables', {
            templateUrl: "keeper/Table/tables.html",
            controller: "TablesCtr"
        }) //桌台管理
        .when('/table/regions', {
            templateUrl: "keeper/Table/regions.html",
            controller: "RegionsCtr"
        }) //桌台管理
        .when('/table/region/:id', {
            templateUrl: "keeper/Table/regionDetail.html",
            controller: "RegionDetailCtr"
        }) //桌台管理
        .when('/table/reserve', {
        ...
        .when('/staff/staffedit/:guestid', {
            templateUrl: "keeper/Staff/staffedit.html",
            controller: "StaffeditCtr"
        }) //员工编辑
        .when('/staff/staffoperation/:guestid', {
            templateUrl: "keeper/Staff/staffoperation.html",
            controller: "Staffoperation"
        }) //权限管理
        .when('/staff/staffadd', {
            templateUrl: "keeper/Staff/staffadd.html",
            controller: "StaffaddCtr"
        }) //员工添加
        .when('/comment', {
            templateUrl: "keeper/comment.html",
        ...
        .when('/customer', {
            templateUrl: "keeper/customer/index.html",
        ...
        .when('/rule', {
            templateUrl: "keeper/rule/index.html",
            controller: "RuleCtr"
        }) //营销规则列表
        .when('/rule/front', {
            templateUrl: "keeper/rule/front/index.html",
            controller: "RuleFrontCtr"
        }) //首页广告
        .when('/rule/coupon', {
            templateUrl: "keeper/rule/coupon.html",
        ...
        .when('/rule/member', {
            templateUrl: "keeper/rule/member.html",
        ...
        .when('/devices', {
            templateUrl: "keeper/devices/index.html",
        ...
        .when('/mall/dish/add', {
            templateUrl: "keeper/mall/dish/add.html",
...
app.controller('ShopCtr', function ($scope, $location, $filter) { //店铺
    ...
    $scope.pageChange = function () {
        console.log('33');
        console.log($scope.view.leftIndex);
        console.log($scope.groups)
        url = "/materials";
        if ($scope.view.leftIndex >= 0) {
            url = "/materials/group/" + $scope.groups[$scope.view.leftIndex].id + "/materials"
        }
    ...
    $scope.submitSetting=function(){
        console.log('提交111');
        console.log($scope.settingPicUrls)
        // console.log($scope.shop)
        // return false;
       let pp=new Promise(function(resolve){
        let json=$scope.settingPicUrls;
        resolve(json)
       })
       pp.then(function(json){
        var data = ajaxSendFn($scope.settingPicUrls, "/shops/shop/setting", "POST");
        if (data.code == 200) {
             $scope.settingPicUrls=ajaxSendFn({}, "/shops/shop/setting", "GET").result;
            alert('操作成功')
        } else {
        ...
        if ($scope.view.select >= 0) {
            if($scope.state=='addShopImg'){
                ...
                var data = ajaxSendFn({
                    ...
                    display:$scope.settingPicUrls.display
                }, "/shops/shop/setting", "POST");
                if (data.code == 200) {
                    $scope.settingPicUrls=ajaxSendFn({}, "/shops/shop/setting", "GET").result;
                    $("#addpic").modal("hide");
                    alert('选择成功')
    ...
    console.log(editor)
    editor.create();
    var instruction= ajaxSendFn({}, "/shops/shop/introduction", "GET").result || [{}];
  
    // console.log(instruction.content)
    $scope.showIntroductor = function () {
        $('#introduction').modal('show');
        // 富文本编辑器赋值
        editor.txt.html(instruction.content)
    }
    ...
    function init() {
        ...
        $scope.view.introduction =instruction;

        // console.log($scope.view.materials)
    }
  
    $scope.copyLink = function (index) {
        var Url2 = document.getElementById("biao" + index);
        let url=Url2.value;
        if(url.indexOf('gif')<0&&url.indexOf('_org')<0){
            // let index=url.lastIndexOf('')
          let arr= url.split('.');
        //   console.log(arr[arr.length-2])
          arr[arr.length-2]+='_org';
    ...
    // console.log($scope.view.introduction)
    $scope.view.videos = ajaxSendFn({}, "/shop/videos/shop", "GET").result || [{}];

    $scope.introductionFn = function () {
        var jsons = {};
        jsons.content=editor.txt.html();
        // jsons.title=$scope.post.title;
...
app.controller('ShopAdd2Ctr', function ($scope, $location) {
    ...
    $scope.addBoxFn = function () { //
        ...
        if ($scope.tem.box.seatNum > 0) {} else {
            return
        }
...
app.controller('StaffShowCtr', function ($scope, $location) {
    ...
    $scope.view = {
        staffs: ajaxSendFn({}, constUrlDict["staffs-shop"], "GET").result || {}
    };
    ...
    $scope.pageChange = function () {
        console.log('44')
        var json = {
            page: $scope.view.staffs.page,
            count: $scope.view.staffs.count
        };
    ...
    $scope.dutySwitch = function () {
        ...
        if (confirm(str)) {
            var data1 = ajaxSendFn({
                "enabled": !$scope.view.setting.enabled
            }, "/duties/state", "POST");
            if (data1.code == 200) {
...
app.controller('DutyEditCtr', function ($scope, $location, $routeParams, $filter) {
    ...
    $scope.init = function () {
        $scope.view.tables = ajaxSendFn({
            date: $routeParams.date
        }, "/duties/duty/day", "GET").result || {};
    }
    $scope.init();
...
app.controller("ShopBusinesstimeCtr", function ($scope, $filter, $location) {
    ...
    $scope.delFn = function (a) {
        ...
        $scope.view.table[a] = {
            ...
            endTime: '',
            show: 0,
            buname: $scope.view.table[a].buname
        };
    }
})
...
app.controller("RuleFrontCtr", ["$scope", "$http", "$filter", "$document", "$compile", "$rootScope", function ($scope, $http, $filter, $document, $compile, $rootScope) {
    $scope.config.breadset = [{ //
        name: "广告管理",
        href: indexUrl + "/leeper.html#/rule/ads",
        class: "front"
    }, {
        name: "广告管理"
    }]; //面包屑;
...
app.controller('TagCtr', function ($scope, $location, $http) { //shoplist over
    ...
    $scope.name="";
    $scope.data =[];
    $scope.data = ajaxSendFn({}, "/dishes/tags", "GET").result;
    console.log($scope.data )
    $scope.addTag=function(){
        if($scope.name.trim()==''){
            alert('请输入名称')
        }else{
          let   send1 = ajaxSendFn({
              name:$scope.name
          }, "/dishes/tags", "POST");
            if (send1.code == 200) {
                $scope.name="";
    ...
    $scope.putTag=function(item){
        if(item.name.trim()==''){
            alert('请输入名称')
        }else{
          let   send1 = ajaxSendFn({
              name:item.name
          }, "/dishes/tags/"+item.id, "PUT");
            if (send1.code == 200) {
                alert('修改成功')
                $scope.data = ajaxSendFn({}, "/dishes/tags", "GET").result;
            } else {
    ...
    $scope.delTag=function(id){
        ...
        var send2 = ajaxSendFn({}, "/dishes/tags/"+id, "DELETE");
        if (send2.code == 200) {
            $scope.data = ajaxSendFn({}, "/dishes/tags", "GET").result;
        } else {
            alert(send2.message);
...
app.controller('DishCtr', function ($scope, $location, $http) { //shoplist over
    ...
    // 同步菜品
    $scope.synchDishes = function () {
            var data = ajaxSendFn({}, "/dishes/sync", "GET");
            if (data.code == 200) {
                alert('成功');
            } else {
                alert(data.message);
        ...
        // 清空菜品
        $scope.deleteDishes = function () {
            if (!confirm("一旦清空将不可恢复，是否确认清空？")) return false;
            var data = ajaxSendFn({}, "/dishes/shop", "DELETE");
            if (data.code == 200) {
                $scope.posts.dishes = [];
