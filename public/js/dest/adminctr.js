...
app.controller('RuleAdd2Ctr', ['$scope', '$location', '$filter', '$routeParams', 'CouponFactory', function ($scope, $location, $filter, $routeParams, couponFac) { //shoplist over
    ...
    $scope.ruleFn = {
        ...
        stateFn: function (a, b, c, d) { //key,state,show,点击添加规则时调用
            ...
            if (c) {
                console.log('111111')
                console.log(aRe) //MEMBER
                console.log($scope.view.rules)
                console.log($scope.view.rules[aRe])
...
app.controller("RuleNonParticipationCtr", ["$scope", "$http", "$filter", "$document", "$compile", "$rootScope", function ($scope, $http, $filter, $document, $compile, $rootScope) {
    ...
    $scope.sendJsons = function () {
        ...
        console.log(json);
        // return false;
        // if ($scope.post.items.length) {
        //     json.items = [];
        //     angular.forEach($scope.post.items, function (value, key) {
        ...
        //     if(i.setmeals.length>0&&i.kinds.length>0&&i.dishes.length>0){
        //         unitJson.push(i)
        //     }
        // }
...
app.controller('RuleuEditorCtr', ['$rootScope', '$scope', function ($rootScope, $scope) { //shoplist over
    ...
    $scope.send = function () {
        ...
        json.content = "<html><head><meta name='viewport' content='initial-scale=1, maximum-scale=1'/><title>" + $scope.post.title + "</title></head><body><div style='" + $("#div1").attr("style").substr(33) + "'>" + editor.txt.html() + "</div></body></html>";
        if ($scope.index) {
...
app.controller('CustomerUpdateCtr', function ($scope, $routeParams, $filter, $http) { //
    ...
    $scope.send = function () {
        ...
        json.currentPoints = $scope.view.user.currentPoints;
        json.currentCharge = $scope.view.user.currentCharge
        console.log(json);
